...
URL_LIST="https://github.com/manuelrn/Tvheadend_Movistar-Spain/raw/master/files/Configuracion_Tvheadend_$REMOTE_LIST_VERSION.tar.xz" 2>>Tvheadend_Movistar-Spain.log

REMOTE_GRABBER_VERSION="$(curl -fLs https://github.com/manuelrn/Tvheadend_Movistar-Spain/raw/master/version.txt | grep ^"GRABBER_VERSION" | cut -d'=' -f2)" 2>>Tvheadend_Movistar-Spain.log
URL_GRABBER="https://github.com/manuelrn/Tvheadend_Movistar-Spain/raw/master/files/tv_grab_movistar-spain" 2>>Tvheadend_Movistar-Spain.log
...
if [ "$INSTALL_GRABBER" = true ]; then
	...
	if [ $SYSTEM -eq 2 ]; then
		...
		sed -i 's/"modules": {/"modules": {\n\t\t"\/usr\/bin\/tv_grab_movistar-spain": {\n\t\t\t"class": "epggrab_mod_int_xmltv",\n\t\t\t"dn_chnum": 0,\n\t\t\t"name": "XMLTV: Movistar+",\n\t\t\t"type": "Internal",\n\t\t\t"enabled": true,\n\t\t\t"priority": 3\n\t\t},/g' $TVHEADEND_CONFIG_DIR/epggrab/config 2>>Tvheadend_Movistar-Spain.log
	else
